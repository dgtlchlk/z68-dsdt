 ...
 *
 * Disassembly of ./dsdt.aml, Fri Jan 25 01:17:09 2013
 *
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x0000A9C0 (43456)
 *     Revision         0x02
 *     Checksum         0x25 **** Incorrect checksum, should be 0xA3
 *     OEM ID           "ALASKA"
 *     OEM Table ID     "A M I"
...
DefinitionBlock ("./dsdt.aml", "DSDT", 2, "ALASKA", "A M I", 0x00000015)
{
    ...
    Name (VGAF, One)
    OperationRegion (GNVS, SystemMemory, 0xDF3B3E18, 0x01CD)
    ...
    Scope (_SB)
    {
            ...
            Device (LPCB)
            {
                ...
                Scope (\_SB)
                {
                    Name (LUCD, Buffer (0x7D)
                    {
                        ...
                        /* 0010 */    0x00, 0x00, 0x00, 0x94, 0xB4, 0x65, 0x61, 0x30, 
                        ...
                        /* 0030 */    0x65, 0x38, 0x31, 0x36, 0x36, 0x37, 0x38, 0x35, 
                        /* 0038 */    0x34, 0x65, 0x38, 0x32, 0x64, 0x63, 0x32, 0x63, 
                        /* 0040 */    0x30, 0x63, 0x31, 0x32, 0x39, 0x32, 0x63, 0x36, 
                        /* 0048 */    0x65, 0x34, 0x32, 0x61, 0x32, 0x66, 0x63, 0x64, 
                        /* 0050 */    0x65, 0x62, 0x65, 0x63, 0x39, 0x30, 0x30, 0x30, 
                        /* 0058 */    0x30, 0x31, 0x44, 0x30, 0x35, 0x63, 0x65, 0x64, 
        ...
        Scope (\_GPE)
        {
            ...
            Method (_L0B, 0, NotSerialized)
            {
                ...
                Notify (\_SB.PCI0.RP08.BR10, 0x02)
                Notify (\_SB.PWRB, 0x02)
            }
        }
    ...
    Scope (_SB.PCI0.LPCB)
    {
        OperationRegion (CPSB, SystemMemory, 0xDF3C5F98, 0x10)
    ...
    Scope (_GPE)
    {
        ...
        {
            Store (\_SB.GNVS (0x8380), Local3)
            Store (GCPT (), Local0)
            Store (OVTL, Local1)
            Store (OVTH, Local2)
            If (LEqual (And (Local3, 0x0F), Zero))
            {
                Store (Zero, EMOD)
            }

            If (LEqual (And (Local3, 0xF0), 0x10))
            {
                If (LAnd (LGreater (Local0, Local2), LNotEqual (EMOD, One)))
                {
                    \AMW0.AMWN (0x00030005)
                    Store (One, EMOD)
                    Or (And (Local3, 0xF0), One, Local3)
                    \_SB.SNVS (0x8380, Local3)
                }

                If (LAnd (LLess (Local0, Local1), LNotEqual (EMOD, 0x02)))
                {
                    \AMW0.AMWN (0x00010005)
                    Store (0x02, EMOD)
                    Or (And (Local3, 0xF0), 0x02, Local3)
                    \_SB.SNVS (0x8380, Local3)
                }
            }
